...
def enbossNSC1(this):
	if this.frame%4==0:
		tmp=orbit.quickcreate(0,point=(random.random()*globe.playrc.width+globe.playrc.left,globe.playrc.top),theta=180+30*random.random()-15,speed=5,bltype=lbltype["blueball"])
		scgame.blmanager.create_enbl(lbltype["blueball"],tmp)
...
def update(time):
	...
	if time>=60 and time<=240:
		...
		if time%45==0:
			...
			if time<=2400:
				tmp=orb["p3e1"].copy()
				tmp.fr=((time-2160)/45*globe.playrc.width/6,globe.playrc.top)
				tmp.tg=(tmp.fr[0],100)
				tmp.refresh()
			else:
				tmp=orb["p3e1"].copy()
				tmp.fr=((6-(time-2400)/45)*globe.playrc.width/6,globe.playrc.top)
				tmp.tg=(tmp.fr[0],200)
				tmp.refresh()
		...
		if time%5==0:
			tmp3=orb["p4e2"].copy()
			tmp3.point=((1+random.random())*(globe.playrc.width/2)+globe.playrc.left,200+30*random.random())
