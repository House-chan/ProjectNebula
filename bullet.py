...
def bl_inter_collide(embullet):
	...
	if type(area) == float or type(area) == int:
		...
		if abs(cos(theta / 180.0 * pi) * disx + sin(theta / 180.0 * pi) * disy) < rctuple[3] / 2 and abs(sin(theta / 180.0 * pi) * disx - cos(theta / 180.0 * pi) * disy) < rctuple[2] / 2:
			return True
	return False
...
class BulletManager(object):
	...
	def __init__(self):
		...
		self.plimg.append(cache.cache_set_alpha(cache.cache_rotate(globe.mgame.rsmanager.anime["ziji"][5], 90), 128))
		self.plimg.append(cache.cache_set_alpha(cache.cache_rotate(globe.mgame.rsmanager.anime["ziji"][6], 90), 128))
